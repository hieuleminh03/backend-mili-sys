    ...
    public function register(): void
    {
        ...
        $this->renderable(function (Throwable $e, $request) {
            ...
            if ($e instanceof \Illuminate\Database\QueryException) {
                ...
                if ($e->getCode() == 23000) {
                    ...
                    if (strpos($e->getMessage(), 'Duplicate entry') !== false) {
                        ...
                        } else if (strpos($e->getMessage(), 'courses_code_unique') !== false) {
            ...
            if ($e instanceof \Exception && !($e instanceof \Symfony\Component\HttpKernel\Exception\HttpException)) {
                ...
                else if (strpos($e->getMessage(), 'Không thể thay đổi kỳ học') !== false) {
                    $status = 422;
                    $response['errors'] = ['term_id' => ['Không thể thay đổi kỳ học sau khi đã tạo lớp']];
                }
                else if (strpos($e->getMessage(), 'Không thể xóa lớp học') !== false) {
                    $status = 422;
                    $response['errors'] = ['course' => ['Không thể xóa lớp học vì có sinh viên đã đăng ký']];
                }
                else if (strpos($e->getMessage(), 'Thời gian đăng ký đã kết thúc') !== false) {
                    $status = 422;
                    $response['errors'] = ['enrollment' => ['Thời gian đăng ký đã kết thúc cho kỳ học này']];
                }
                else if (strpos($e->getMessage(), 'Lớp học đã đạt giới hạn đăng ký tối đa') !== false) {
                    $status = 422;
                    $response['errors'] = ['enrollment' => ['Lớp học đã đạt giới hạn đăng ký tối đa']]; 
                }
                else if (strpos($e->getMessage(), 'Giới hạn đăng ký không thể nhỏ hơn số sinh viên') !== false) {
                    $status = 422;
                    $response['errors'] = ['enroll_limit' => [$e->getMessage()]];
                }
                else if (strpos($e->getMessage(), 'Sinh viên đã đăng ký') !== false) {
                    $status = 422;
                    $response['errors'] = ['enrollment' => ['Sinh viên đã đăng ký lớp học này']];
                }
                else if (strpos($e->getMessage(), 'Thời gian nhập điểm chưa bắt đầu') !== false) {
                    $status = 422;
